...
func newMapSession(privateNodeKey key.NodePrivate, nu NetmapUpdater, controlKnobs *controlknobs.Knobs) *mapSession {
	ms := &mapSession{
		...
		onSelfNodeChanged: func(*netmap.NetworkMap) {},
		changeQueue:       make(chan response),
...
func (ms *mapSession) run() {
	...
	for {
		...
		case change := <-ms.changeQueue:
			ms.handleNonKeepAliveMapResponse(ms.sessionAliveCtx, change.response)
...
func (ms *mapSession) addRespToQueue(resp response) error {
	select {
