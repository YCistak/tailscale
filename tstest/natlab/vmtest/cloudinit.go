...
func writeLinuxDNSModeSetup(ud *strings.Builder, mode DNSMode) {
	...
	case DNSOpenresolv:
		// Mask systemd-resolved and install upstream openresolv, so
		// resolvconfStyle() in net/dns/resolvconf.go finds a resolvconf that
		// isn't Debian's and dnsMode() selects the "openresolv" manager. The
		// files come from the vnet fileserver; see Env.prepareOpenresolv.
		fmt.Fprintf(ud, "  - [\"/bin/sh\", \"-c\", \"systemctl disable --now systemd-resolved 2>/dev/null || true\"]\n")
		fmt.Fprintf(ud, "  - [\"/bin/sh\", \"-c\", \"systemctl mask systemd-resolved 2>/dev/null || true\"]\n")
		// resolvconf creates the key directory only when a snippet is
		// registered, and a missing directory behaves differently (exit 0, not
		// 2), so provision it empty. See [DNSOpenresolv].
		fmt.Fprintf(ud, "  - [\"mkdir\", \"-p\", \"%s\", \"%s\"]\n", openresolvLibexecDir, openresolvKeyDir)
		for _, f := range openresolvInstall {
			fmt.Fprintf(ud, "  - [\"/bin/sh\", \"-c\", \"curl -v --retry 10 --retry-delay 2 --retry-all-errors -o %s http://52.52.0.6/%s 2>&1\"]\n", f.guestPath, f.servePath)
			if f.exec {
				fmt.Fprintf(ud, "  - [\"chmod\", \"+x\", \"%s\"]\n", f.guestPath)
			}
		}
